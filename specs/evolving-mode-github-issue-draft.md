...
Consumption is resolved statically at process creation — by walking the same condition graph the planner searches, so chain membership follows the planner's own assignability rules — and applied through `Blackboard.hide` at completion; there is no per-occurrence tracking and no consumer-facing activation API. Hiding is identity-based, honoring the hide contract: consuming an occurrence hides exactly that object, and a distinct but equal occurrence remains a live request. Domain records need no occurrence ids for lifecycle purposes. The observable contract is consume once, do not complete the process, and allow a later occurrence to run again.
...
- **Confirm identity-based `Blackboard.hide`.** The hide KDoc says "hide this object", but the implementation hid by equality, which coalesced distinct-but-equal occurrences and let a consumed output render a later equal output invisible, blocking the next episode (phase-1 tests). The phase-1 branch makes `InMemoryBlackboard` hiding identity-based to match the documented contract, with the full module suite staying green. Maintainer confirmation wanted, since this touches platform behavior beyond episodes.
...
The framework owns the point after goal satisfaction and before process completion. That is where it can hide both the exact request and the newly produced satisfying output before ordinary selection resumes. An action cannot reliably perform both halves itself because its satisfying output is added after it returns. The baseline demonstrates that hand-rolling this today requires a janitor action, an archive type, `canRerun`, two hide calls, and coordinated action values — and a multi-step path additionally owes one hide per intermediate, or a leftover intermediate re-satisfies the goal without a new request. The `evolving-mode-phase-1` branch carries a reference implementation driven by a 31-test suite exercising these criteria, including regressions from an adversarial review pass.
