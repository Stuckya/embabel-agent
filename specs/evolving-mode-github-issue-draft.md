...
Maintainer input would be helpful on these decisions. Earlier open questions now decided and test-pinned: episodic is ambient (`withEvolving` derives every rule; the stance shipped), the completion model is the committed objective (the founding episode is terminal; incidental achievements resume), no rule spellings remain because no rule surface remains — the razor deleted it — and **execution has one model**: the in-process rung was built, pinned, and deleted, because every guarantee the child boundary gives structurally had to be policed by mechanism inside the parent's walk, and the walk itself was the hazard (there is no guarantee a planner walks A to Z; a child has no walk to guarantee). Hot loops too cheap for a child per occurrence were the rung's remaining argument, and measurement dissolved it: dispatch spawns no thread, the snapshot is shallow reference copies, and per-step planning over the chain subset is smaller than the parent's full-system replan. Git history preserves the deletion round's receipts.

Decided forward commitments, so future work lands on a named shape rather than a re-litigated one:

- **Interruption is phase 2's contract.** A new evolution interrupts a long chain through publication-time wake plus cooperative cancellation targeted at the deepest active descendant — never through an in-parent preemption rung. Until phase 2, a chain runs whole per dispatch.
- **Hot loops get a designed rung later, not a policed one now.** If a real case needs episodes cheaper than a child per occurrence, the shape is lightweight views/receipts over instance-scoped binding — a purpose-built primitive, not a revival of the deleted opt-out.
- **Provenance and consumability become tags when the platform is ready**: every board write stamped with its episode id (observed) and a consumability tag defaulted from derivation, with a per-instance call-site override (`addStanding`/`addProduct`-shaped) as the explicit escape hatch. The invariant that keeps tagging safe: tags are completion bookkeeping, never planning facts — boundaries stay structural, never demarcated.
...
- **Parallel actions inside one evolving process — resolved by the rung deletion.** Parallel episodes are parallel processes; the in-process variant no longer exists to be half-supported. Frame-action concurrency inside the evolving parent stays serial in phase 1 and is admission-width work in phase 2. Instance-scoped binding views remain the deeper platform primitive worth naming for the hot-loop future.
