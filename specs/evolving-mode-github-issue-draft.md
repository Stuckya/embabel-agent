...
> and framework-dispatched child execution as the one execution model, over
...
record Evolving(GoalTarget objective) {}
...
An episode is one bounded plan-execute-complete cycle, atomic per dispatch: the chain runs whole in its child, and standing work interleaves between episodes by value. Three runtime mechanisms carry the contract.
...
The lifecycle is per occurrence and the declared goal is reusable. Completing an episode never completes the process. In evolving mode every other declared goal is a frame achievement - recorded, withdrawn from planning, never terminal - and only the committed objective completes the process; outside evolving mode declared goals keep today's behavior. Episodes are independent, not idempotent: duplicate requests are distinct occurrences and each is handled, while standing state legitimately advances across episodes.

The contract has one execution model, following AIMA ch 2's observation that environments are episodic at higher levels than individual actions: **every episode executes in a framework-synthesized child process**. Two of the book's own classifications validate the boundary. A child's snapshot world is static and fully observable in ch 2's taxonomy under today's synchronous dispatch — nothing on the blackboard can change under the child but by its own actions, though the snapshot shares instances by reference and application-owned conditions reading external state stay live — which is the environment class where classical full-path planning is sound: the child boundary manufactures the world its planner assumes, and phase 2's async dispatch narrows the claim. And the children are multibody, not multiagent (§11.4, p. 425): "a multibody problem is still a 'standard' single-agent problem as long as the relevant sensor information collected by each body can be pooled … to form a common estimate of the world state" — merge-back is that pooling condition, which is the soundness argument for the whole child model. Dispatch is a selection outcome, not an admission side-effect: each tick the planner weighs the frame's best plan against every active episode, each worth its goal's declared value, and the winner executes — one execution per tick, so standing work and episodes interleave exactly as value dictates, with selection owned by the planner throughout. The framework projects the winning episode's derived chain into a child process it synthesizes, runs it, and merges the outcome home. The developer writes one ordinary agent and calls evolve; dispatch, isolation, and merge-back are invisible. The child boundary draws structurally what mechanisms would otherwise have to police — begin-episode is the child's birth, end-episode its completion, and the planner cannot wander out of an episode that is the whole world it lives in: there is no walk from A to Z to guarantee, because no walk crosses the boundary. Pairing rides the snapshot for free, because queued occurrences hidden in the parent stay hidden in the child. A stale satisfying output cannot vacuously complete a child, because execution history is process-scoped — and the same scoping makes `canRerun = false` occurrence-scoped: each child runs a non-rerunnable action once, so occurrences stay independent instead of the mission going one-shot. The request window is structural too: same-class competitors are hidden in the child's snapshot, so binding by type resolves the occurrence the episode holds. An `evolve` from inside a child delegates up the tower to the nearest evolving ancestor, so self-chaining authors exactly as frame publication does — and terminal evaluation precedes admission on every arrival path: plannable founding work runs before a fresh or queued occurrence re-arms its rule. The merge-back contract was decided empirically: everything the chain wrote comes home, consumable-typed instances consumed at completion and undeclared standing writes surviving — the strict declared-outputs-only alternative was tried and rejected when it stranded the batch tally in dead children and made self-chaining spin unboundedly. That experiment also produced the brake: dispatches spend the parent's action budget, so an unbounded chain terminates instead of spawning forever. Nothing is proven before dispatch, and failure is welcome: a blocked child is a record of exactly what was missing, the input Open Evolving's author will one day consume, and a gate that predicted stalls would starve that author of evidence. The defense against futile repetition (AIMA 3e p. 425 note 5) is observational instead: a chain that ran and blocked is never re-attempted into a world that has not changed, so a blocked episode costs one observed child per changed world — its partial work dying with its own blackboard — never the budget. A crashed child is contained and retries freely — a transient fault leaves the blackboard unchanged, so waiting would gain nothing — while a blocked child waits for new facts; both leave the occurrence unconsumed, bounded by the parent's action budget. Because nothing gates on proof, a chain no planner could prove ahead of time is still runnable: under an inherited value-walking planner, a chain whose interior condition only turns true mid-run walks to its goal — the run is the proof — while under a classical planner the same chain waits as one recorded attempt, evidence for the author rather than work refused. And the resolution loop closes with the same machinery: a blocked episode resumes when any later evolution's lasting products change the world, which is the whole feature in miniature — friction, new evolution, resolution. Framework children inherit the parent's options - budget limits, identities, context, and the declared planner, which the framework never overrides - minus the evolving declaration; their counters are fresh against the parent's limits, each dispatch spends one parent action, and hierarchical draw-down is a named platform follow-on. Manual dispatch through `createChildProcess` with declared options remains available for explicit crews, and declared options make the tower unbounded: an evolving child inside an evolving parent, composed deliberately and never by inheritance. An in-process opt-out existed and was deleted: it re-ran episode chains inside the parent's walk, where every guarantee above had to be policed by mechanism — a request window per action, attribution by blackboard diff, a liveness carve-out in the gate, and the HYBRID value traps the AIMA cross-check catalogued. Deleting it removed the `EpisodeExecution` declaration and the policed walk with it; hot loops too cheap for a child per occurrence get a purpose-designed lightweight mechanism later, rather than an inherited opt-out. Under a concurrent process evolving mode executes serially in phase 1, and concurrent width is phase 2's admission-width work. Parallel same-rule episodes are parallel children, where a bounded admission width — how many may run at once — encodes the capacity of what the chains borrow (§11.1's `Inspectors(2)` aggregation). Width defaults to one and its generalization ships with phase 2's async dispatch.
...
The phase-1 implementation should not introduce keyed goal-instance satisfaction. Domains needing key-specific behavior should use existing types, conditions, bindings, and action inputs. Episode rearm needs no reset machinery: execution history is process-scoped, so `canRerun = false` means once per occurrence's child — each occurrence runs it exactly once and occurrences stay independent, while within one chain the action still fires only once.
...
Maintainer input would be helpful on these decisions. Earlier open questions now decided and test-pinned: episodic is ambient (`withEvolving` derives every rule; the stance shipped), the completion model is the committed objective (the founding episode is terminal; incidental achievements resume), no rule spellings remain because no rule surface remains — every candidate spelling restated the goal graph and was deleted — and **execution has one model**: in-process episode execution was built, pinned, and deleted, because every guarantee the child boundary gives structurally had to be policed by mechanism inside the parent's walk, and the walk itself was the hazard (there is no guarantee a planner walks A to Z; a child has no walk to guarantee). Hot loops too cheap for a child per occurrence were its remaining argument, and measurement dissolved it: dispatch spawns no thread, the snapshot is shallow reference copies, and per-step planning over the chain subset is smaller than the parent's full-system replan. Git history preserves the deletion round's receipts.

Decided forward commitments, so future work lands on a named shape rather than a re-litigated one:

- **Interruption is phase 2's contract.** A new evolution interrupts a long chain through publication-time wake plus cooperative cancellation targeted at the deepest active descendant — never through in-parent preemption. Until phase 2, a chain runs whole per dispatch.
- **Hot loops get a purpose-designed mechanism later, not a policed one now.** If a real case needs episodes cheaper than a child per occurrence, the design direction is lightweight per-episode views over instance-scoped bindings — a purpose-built primitive, not a revival of the deleted opt-out.
- **Provenance and consumability become tags when the platform is ready**: every blackboard write stamped with its episode id (observed) and a consumability tag defaulted from derivation, with a per-instance call-site override (`addStanding`/`addProduct`-shaped) as the explicit escape hatch. The invariant that keeps tagging safe: tags are completion bookkeeping, never planning facts — boundaries stay structural, never demarcated.
- **Derivation becomes introspectable in phase 3**: the derived scope (chains, evolved-eligible types, consumables, exclusions with reasons) exposed as public API, so developers pin their derivation in their own tests and a graph edit that changes inference fails a test instead of silently re-gating an action.
- **Nothing is proven before dispatch — failure is welcomed.** The feasibility gate was built, then deleted by experiment: 70 of 73 pins held without it, and the gate turned out to be friction *suppression* — a chain never attempted generates no evidence of what is missing, which starves the author Open Evolving exists to consult. Selection and routing rank by declared goal values; the child's run is the only verdict on a chain; a blocked child is a recorded obstruction, keyed to the world it blocked in, and the world-change retry is precisely how an author's contribution will one day resume it: the author is a world-changer plugged into the observation loop, no new machinery. The known cost, priced deliberately: selection no longer sees chain costs, only declared goal worth.
- **The declared planner is inherited, never overridden.** Children run whatever the developer declared, pairing goal and all; the old GOAP forcing was the framework knowing better than a declaration. The escalation ladder — a classical child proves it, a value-walking child explores it, an author changes the world — is future work gated on a planner fix flagged to the maintainer: value-tie selection starves diamond chains by re-picking the same rerunnable branch instead of preferring un-run actions.
- **Derivation is at-need and full-depth — the shallow alternative was tried and refuted.** An experiment replaced per-goal path analysis with agent-level signature reads (no chains, no exclusion set, no per-goal consumption scoping) and the test battery convicted every shortcut at once: agent-wide consumption made completion hide the process's own products and missions looped to budget death, the emptied exclusion set let the parent's value planner run episode work itself, and goals that should be excluded admitted and consumed facts they could never serve. The lesson, priced: per-goal depth is load-bearing for consumption, exclusion, and eligibility - but none of it needs to run before a fact asks. Derivation is deduction from declarations at the moment of need, and the moment of need is the first arrival, not construction.
- **Eligibility is some-path, and the cycle check earned its place.** Required-on-every-path eligibility was replaced by the chain's own unmet inputs: the every-path claim was blind to conditions and a dead branch could empty it, excluding drivable goals. Goals sharing producers now contest each arrival by value. In the same round, deleting the two-action-cycle half of the standing-state check was tried and refuted by a pin written first: hiding cycle-maintained state blocked every chain that reads it, so the check stays with its receipt.
- **Contested intent gets a call-site home in phase 2**: an optional routing target on `evolve` (`toward`-shaped), the output-side sibling of designation, for publishers that mean a specific winner. Value-based routing stays the default.

- **Where does evolve live, and what does its rejection throw?** `evolve` ships on the `AgentProcess` interface; `ctx.evolve()` on `ActionContext` is the remaining proposed home, and every test reaching through `context.agentProcess` is the evidence it is overdue. The unroutable rejection is `IllegalArgumentException` today, carrying the exclusion reasons; a distinct type would let publishers and phase-5 machinery catch it specifically.
...
- **Re-derivation on a live process — the timing half is built.** Derivation now runs when a fact first asks for it, against the declarations as they stand then, and the derived result is held behind an invalidation seam: when a future scope change invalidates it, the next arrival derives against the changed declarations, live, no restart. What remains for Open Evolving is the scope mutation itself and rule identity across derivations — likely keyed by goal name.
- **Parallel actions inside one evolving process — resolved by deleting in-process execution.** Parallel episodes are parallel processes; the in-process variant no longer exists to be half-supported. Frame-action concurrency inside the evolving parent stays serial in phase 1 and is admission-width work in phase 2. Instance-scoped binding views remain the deeper platform primitive worth naming for the hot-loop future.
...
- **Episode / goal episode** - one occurrence of a request, planned to goal completion. Completion consumes the request and whatever the episode made, which is what makes the next occurrence independent (AIMA ch 2: "the next episode does not depend on the actions taken in previous episodes"). At runtime an episode is a framework-internal object holding its request, its attributed consumables, and its state (`PENDING`, `ACTIVE`, `COMPLETED`). In AIMA terms it is one pass of the plan-execute-replan loop (§11.3.3), and its request is its percept (ch 2). Atomic per dispatch: the chain runs whole in its child, and standing work interleaves between episodes. Independent, not idempotent: duplicate requests are distinct occurrences. Work stream 7 explores recurring episodes — no request — for pure-GOAP standing work.
...
- **Child execution** - the one execution model: the framework synthesizes a child process from the derived chain, runs the episode there, and merges the outcome home. The child boundary draws structurally what mechanisms would otherwise police. Manual dispatch through `createChildProcess` remains for explicit crews and for the tower, since framework children are default-mode and an evolving child requires declared options.
...
- A derived rule's eligible arrival types MUST be the default-binding inputs the goal's chain reads but cannot produce itself — inputs the planner cannot manufacture. The accepted trade, priced: when a run's chosen path does not read the arrival, completion still consumes it by identity. The named future remedy is observational: the child's run can report whether any executed action bound the request's type, and completion would consume only a read request. Goals sharing producers contest each arrival and value decides, exactly as shared input types route in default mode.
...
1. an ephemeral process MUST NOT evolve: episodes execute in child processes, which require the persistence the ephemeral declaration disclaims, and the conflict fails at construction;
...
The framework owns the point after goal satisfaction and before process completion. That is where it consumes the exact request and the attributed consumables before ordinary selection resumes. An action cannot reliably perform both halves itself because its satisfying output is added after it returns. The baseline demonstrates that hand-rolling this today requires a janitor action, an archive type, `canRerun`, two hide calls, and coordinated action values. The reference implementation was driven test-first through five adversarial rounds: the declared-policy dialect (implemented, pinned, then deleted when every field it declared turned out to restate the goal graph), the derived mode, the founding episode, two external review passes, the subagent proof, and framework dispatch as the default execution; git history preserves each stage.

Action failure does not complete the episode: a failed attempt leaves the request unconsumed, and the next tick's selection respawns a fresh child. Existing failure and replanning behavior remains in force; retry limits and backoff are separate concerns.
...
- `canRerun = false` scopes to the occurrence: each child runs a non-rerunnable action once, and occurrences stay independent
- the concurrent process shares the contract: evolving mode executes serially in phase 1, one action per tick
...
- the founding loop: a blocked episode is resolved by a later evolution whose lasting products change the world, a chain no planner can prove ahead of time completes under a value-walking child and waits as recorded friction under a classical one, two equal facts are two independent episodes because identity decides, a permanent crash retries exactly to the budget, and a child-published follow-up records its dispatching episode as its cause
...
- `ctx.evolve()` on `ActionContext` (queued ergonomics; `AgentProcess.evolve` shipped)
- pub/sub fan-out
...
- parallel action execution inside one evolving process; parallel episodes are parallel children
...
Timing matters here. Blackboard publication and normal planning are tick-confined, but a planning tick may not occur while one long action blocks — the exact case this sub-issue exists for. Under default child execution the in-flight action belongs to the deepest active descendant, not the evolving parent — the parent is dispatch-blocked, not action-blocked — so the cancellation signal must target the active descendant transitively, the downward mirror of evolve's upward delegation. An urgent publication landing mid-dispatch is likewise invisible to the running child's snapshot: urgency waits out the chain unless the signal reaches it. The cancellation request therefore cannot wait for the tick. `AgentProcess.terminateAction` is the existing primitive, but this phase must define thread-safe targeting, no-op behavior when no action is active, and a token/checkpoint that remains valid when action work delegates to another thread.
