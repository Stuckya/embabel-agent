...
> **Status: superseded design handoff.** This document records the earlier
> fact-to-runtime-goal and GoalAgenda direction explored by the local throwaway
> POC. Baseline tests and maintainer discussion in issue #1756 established that
> ordinary Embabel planning already makes declared goals plannable as typed
> request objects appear. The current proposal is goal-episode lifecycle:
> nonterminal completion, request/output hiding, and rearm. The Phase 1 core is
> an episode policy on `ProcessOptions`; it does not require a
> new invocation type. The canonical long-form design and sub-issue drafts are in
> `evolving-mode-github-issue-draft.md`.

Draft handoff for the `codex/evolving-mode` work. This document records the
earlier desired public shape after local dogfooding, while keeping the example
domain generic.
