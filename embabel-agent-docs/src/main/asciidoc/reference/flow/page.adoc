...
The blackboard is process-local working memory, not a mutable world-state database or pub/sub event bus.
For future evolving-mode APIs, external async facts should enter a running process through a sanctioned process ingress seam, then become blackboard facts at process seams.
